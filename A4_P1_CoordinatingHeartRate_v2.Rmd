...
path<-("C:/Users/Bruger/Desktop/Programming/Semester 3/asm_7")

folder = file.path(path)

all_list<- list.files(path = folder, recursive = TRUE)
...
for (i in all_list){
  ...
  hplot[n] = ggplot(x, aes(time, HR1)) + geom_line() + geom_line(aes(time, HR2), color = "red")
  rplot[n] = ggplot(x, aes(time, Resp1)) + geom_line() + geom_line(aes(time, HR2), color = "blue")
  
  
  
  assign(paste("HRplot", n, sep = ""), hplot)
  assign(paste("RSPplot", n, sep = ""), rplot)
  
  
  # write file
  
  namefile = paste("Preprocess/preprocess", i, sep = "")
  
  write.csv(x, file = namefile)
...
data = data.frame(study, trial, group, condition, hplot, rplot)


  ID = str_extract(i, "S\\d+")
  study[n]=str_extract(ID, "\\d+")
  d=str_extract(i, "G\\d+")
  group[n] = str_extract(d, "\\d+")
  t=str_extract(i, "T\\d+")
  trial[n]=str_extract(t, "\\d+")
  s=str_extract(i, "\\S")
  condition[n]=str_extract(s, "\\d+")
